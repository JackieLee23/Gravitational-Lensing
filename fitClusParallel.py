# CRK 2021/2/6: revise how linear algebra is handled, to improve robustness
...
class galclass:
    ...
    ##################################################################
    ngal = 0
    dat  = []      # list of [x,y,magnitude, probability]
    ID   = []      # list of ID's for the model components
    p    = []      # lens model parameters
    logflags = []  # flags for whether (Rein,rscale) are logarithmic
    ...
    ##################################################################
    def draw(self, scatter = True):
        iref,m0,b0,a0 = self.ref
        etab,sigb,etaa,siga = self.mod
        # initialize
        self.p = np.zeros((self.ngal,8))
        # positions
        self.p[:,1] = self.dat[:,0]
        self.p[:,2] = self.dat[:,1]
        ...
        #Vary cluster member choice.
        for i in range(len(self.dat)):
            if np.random.rand() < self.dat[i, 3]:
                self.p[i,7] = 1
    ...
    def calcdefbasic(self, xarr):
        return calcdef(self.ID,self.p,xarr,logflags=self.logflags)
...
class defclass:
    ...
    # draw random samples
    ##################################################################
    def draw(self,gal,img,b0,aarr,nran,scatter, basename='',savep=False,useD=False,verbose=True):
        self.nimg = img.nimg
        self.aarr = aarr
        # initialize
        self.marr = np.zeros((len(aarr),self.nimg,2))
...
class lensmodel:
    ...
    def lnP(self,p):
        ...
        UTSU = np.dot(self.img.Umat.T,np.linalg.solve(Ceff,self.img.Umat))
        darr = self.img.xarr - atot
        dtmp = darr.flatten()
        UTSd = np.dot(self.img.Umat.T,np.linalg.solve(Ceff,dtmp))
...
class fitclass:
    ...
    #need this since lambdas aren't picklable
    def lnPMode(self, p):
        return self.lnP(p)[self.mode]
    ...
    def MCrun2(self, myLens, p0=[]):

        with Pool(initializer = setlm, initargs=(myLens,)) as pool:
            ...
            # initialize the sampler
            self.sampler = emcee.EnsembleSampler(self.nwalk,ndim,lnPParallel, pool = pool, args = (self.mode,))
    ...
    def MCrun3(self,p0=[]):
        ...
        self.sampler.run_mcmc(pstart, self.nburn, progress = "notebook")
        
        pos,prob,state = self.sampler.get_last_sample()
        ...
        print("mc: main run")
        self.sampler.reset()
        self.sampler.random_state = state

        # start from end of burn-in and sample many more steps
        self.sampler.run_mcmc(pos, self.nstep, progress = "notebook")
    ...
    def MCrun4(self,myLens, p0=[]):
        ...
        with Pool(initializer = setlm, initargs=(myLens,)) as pool:
            ...
            self.sampler.run_mcmc(pstart, self.nburn, progress=True)
...
###########################################
#Parallelization stuffs
###########################################

#For initialization of lensmodel instances when setting up pools
def setlm(myLens):
    global lm
    lm = myLens


#Same as lensmodel lnp, but moved outside
def lnPParallel(p, mode):
